...
export const createCheckoutSession = async (req, res) => {
	try {
		...
		const lineItems = products.map((product) => {
			const amount = Math.round(product.price * 100); // stripe expects amount in cents thats why we multiply by 100
		...
		res.status(500).json({ message: 'Error in createCheckoutSession controller', error: error.message });
	}
};
...
export const checkoutSuccess = async (req, res) => {
	try {
		...
		if (session.payment_status === 'paid') {
			...
			// create a new order in your database here
		...
		res.status(500).json({ message: 'Error in checkoutSuccess controller', error: error.message });
	}
};
